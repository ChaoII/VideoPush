...
void FFmpegThread::run()
{
    while (!stopped) {
        ...
        if (result >= 0) {
            ...
            msleep(100);
            errorCount_++;
            debug(result, "错误计数", QString("次数: %1").arg(errorCount_));
            if (errorCount_ >= 3) {
                this->reopen();
                continue;
            }
...
bool FFmpegThread::checkFrameSize(AVFrame *frame)
{
    ...
    if (width > 0 && height > 0 && (videoWidth_ != width || videoHeight_ != height)) {
        //先要根据旋转角度调整尺寸
        int w = width;
        int h = height;
        VideoHelper::rotateSize(rotate_, w, h);
        if (isCrop) {
...
bool FFmpegThread::scaleAndSaveVideo(bool &needScale, AVFrame *frame)
{
    ...
    if (isRecord && !getIsFile() && encodeVideo_ == EncodeVideo_None && hardware != "none") {
        needScale = false;
    }

    //非录像阶段必须处于截图和绘制模式下才需要转换
    if (!isRecord && (isSnap || videoMode_ == VideoMode_Painter)) {
        needScale = false;
    }
...
void FFmpegThread::checkAndShowVideo(bool needScale, AVFrame *frame)
{
    ...
    if (isSnap || videoMode_ == VideoMode_Painter) {
        ...
        QImage image(imageData, videoWidth_, videoHeight_, QImage::Format_RGB888);
        if (image.isNull()) {
            return;
        }

        //如果有旋转角度先要旋转
        VideoHelper::rotateImage(rotate_, image);
        if (isSnap) {
            isSnap = false;
            //裁剪期间应用了裁剪滤镜对应的截图有问题
            if (isCrop) {
            ...
            emit receiveImage(image, timer_.elapsed());
        }
    } else {
        //可能有tile滤镜导致产生空帧
        if (frame->width == 0) {
...
void FFmpegThread::decodeVideo0(AVPacket *packet)
{
    ...
    if (realBitRate) {
        FFmpegThreadHelper::checkRealPacketSize(this, packet, frameRate_, realPacketSize, realPacketCount);
    }
...
void FFmpegThread::decodeVideo1(AVPacket *packet)
{
    ...
    //未编码的才存在缓存数据需要清空
    bool videoEncode = saveFile_->getVideoEncode();
    if (!videoEncode) {
        if (needClear) {
            keyFrame = false;
    ...
    if (saveFile_->getIsOk() && !videoEncode && !pushPreview) {
        return;
    }
...
void FFmpegThread::decodeVideo2(AVPacket *packet)
{
    ...
    if (!videoFilter.init || (isSnap && rotate_ > 0)) {
        this->initFilter();
    }
...
void FFmpegThread::decodeAudio1(AVPacket *packet)
{
    ...
    //未编码推流不用继续(可以节约大量CPU/预览还需要继续解码)
    if (saveFile_->getIsOk() && !saveFile_->getAudioEncode() && !pushPreview) {
        return;
    }
...
void FFmpegThread::decodeAudio2(AVPacket *packet)
{
    ...
    //计算直接播放的数据的大小
    int len = av_samples_get_buffer_size(nullptr, pcmChannels, size, pcmSampleFormat, 1);
    //播放声音数据
...
void FFmpegThread::writeFile(AVPacket *packet, bool video)
{
    ...
    //没开启音视频编码就直接写入原始数据包
    if (video) {
        if (!saveFile_->getVideoEncode()) {
            saveFile_->writePacket(packet, videoIndex);
        }
    } else {
        if (!saveFile_->getAudioEncode()) {
            saveFile_->writePacket(packet, audioIndex);
        }
    }
}
...
void FFmpegThread::writeFile(AVFrame *frame, bool video)
{
    ...
    //裁剪滤镜不支持录像存储
    if (isCrop) {
        if (!saveFile_->getIsPause()) {
            saveFile_->pause();
        }
        return;
    } else {
        if (isRecord && saveFile_->getIsPause()) {
            saveFile_->pause();
        }
    }
    ...
    if (video) {
        if (saveFile_->getVideoEncode()) {
            saveFile_->writeFrame(frame, videoIndex);
        }
    } else {
        if (saveFile_->getAudioEncode()) {
            saveFile_->writeFrame(frame, audioIndex);
        }
    }
}
...
void FFmpegThread::initOption()
{
    ...
    if (mediaType == MediaType_Device || mediaType == MediaType_Screen) {
        ...
        FFmpegHelper::initOption(&options, bufferSize, frameRate_);
...
bool FFmpegThread::initInput()
{
    ...
    if (mediaType == MediaType_Device) {
        FFmpegHelper::setVideoCodecName(formatCtx, videoCodecName_);
    }
    ...
    tryOpen = true;
    int result = avformat_open_input(&formatCtx, urlData.constData(), ifmt, &options);
    tryOpen = false;
    if (result < 0) {
        debug(result, "打开地址", "");
    ...
    //封装格式
    formatName_ = formatCtx->iformat->name;
    //校验硬解码
    ...
    QString msg = QString("格式: %1 时长: %2 秒 加速: %3").arg(formatName_).arg(duration / 1000).arg(hardware);
    debug(0, "媒体信息", msg);
    return true;
}
...
bool FFmpegThread::initVideo()
{
    ...
    if (videoIndex < 0) {
        ...
        if (rotate_ != -1) {
            FFmpegHelper::setRotate(videoStream, rotate_);
        }
        ...
        //获取分辨率大小
        FFmpegHelper::getResolution(videoStream, videoWidth_, videoHeight_);
        //如果没有获取到宽高则返回
...
bool FFmpegThread::initAudio()
{
    ...
    if (audioIndex < 0) {
        ...
        audioCodecName_ = audioCodec->name;

        //获取音频相关参数
        profile_ = audioCodecCtx->profile;
        sampleRate_ = audioCodecCtx->sample_rate;
        //发现有些地址居然有6个声道
        channelCount_ = FFmpegHelper::getChannel(audioCodecCtx);

        //部分设备比如海康不支持48000采样率(大华的居然都支持)
        if (mediaType == MediaType_Rtsp && sampleRate_ >= 48000 && mediaUrl.contains("/Streaming/Channels/")) {
            decodeAudio = false;
            audioIndex = -1;
        }
...
void FFmpegThread::initFilter()
{
    ...
    //硬解码或者采集桌面等情况不支持图形滤镜
    if (hardware != "none" || mediaType == MediaType_Screen || mediaUrl.contains("screen-capture-recorder")) {
        this->listGraph_.clear();
    }

    //抓图和绘制模式不用旋转
    int rotate = ((isSnap || videoMode_ == VideoMode_Painter) ? 0 : this->rotate_);
    ...
    if (videoFilter.enable) {
        ...
        videoFilter.listOsd = listOsd_;
        videoFilter.listGraph = listGraph_;
...
QImage FFmpegThread::getImage()
{
    ...
    image = QImage(imageData, videoWidth_, videoHeight_, QImage::Format_RGB888);
    if (image.isNull()) {
        return image;
    }
...
int FFmpegThread::getRotate()
{
    ...
    rotate_ = 0;
    if (videoIndex >= 0) {
        rotate_ = FFmpegHelper::getRotate(formatCtx->streams[videoIndex]);
...
void FFmpegThread::setRotate(int rotate)
{
    ...
    //只有变化了才需要触发
    bool changed = (this->rotate_ != rotate);
    this->rotate_ = rotate;
...
void FFmpegThread::initSaveFile()
{
    ...
    saveFile_->setProperty("cryptoKey", cryptoKey);

    //设置一些弱属性方便其他地方处理
    saveFile_->setProperty("mediaUrl", FFmpegHelper::getUrl(formatCtx));
    saveFile_->setProperty("isFile", getIsFile());

    //告诉保存那边这边传过去的真实的音频数据相关参数
    saveFile_->setProperty("resample", (pcmSwrCtx != nullptr));
    saveFile_->setProperty("sampleRate", pcmSampleRate);
...
void FFmpegThread::recordStart(const QString &fileName)
{
    ...
    if (saveVideoType_ > 1) {
        ...
        if (saveFile_->getIsPause()) {
            isRecord = true;
            saveFile_->pause();
            emit recorderStateChanged(RecorderState_Recording, saveFile_->getFileName());
        } else {
            this->initSaveFile();
            saveFile_->open(this->fileName_);
            if (saveFile_->getIsOk()) {
                isRecord = true;
                emit recorderStateChanged(RecorderState_Recording, saveFile_->getFileName());
            }
...
void FFmpegThread::recordPause()
{
    ...
    if (saveVideoType_ > 1) {
        if (saveFile_->getIsOk()) {
            isRecord = false;
            saveFile_->pause();
            emit recorderStateChanged(RecorderState_Paused, saveFile_->getFileName());
        }
    }
}
...
void FFmpegThread::recordStop()
{
    ...
    if (saveVideoType_ > 1) {
        if (saveFile_->getIsOk()) {
            isRecord = false;
            saveFile_->stop();
            emit recorderStateChanged(RecorderState_Stopped, saveFile_->getFileName());
        }
    }
}
