...
VideoThread *VideoThread::getVideoThread(const WidgetPara &widgetPara, const VideoPara &videoPara) {
    ...
    for (auto videoThread: videoThreads) {
        //桌面采集永远不共享
    ...
    while (!stopped) {
        ...
        if (videoCore == VideoCore_Vlc) {
            ...
            if (mediaType == MediaType_FileLocal) {
                //本地文件需要这里实时读取播放进度
                this->getPosition();
            } else if (isOk && videoMode_ == VideoMode_Hwnd) {
        ...
        if (readTimeout > 0) {
            qint64 offset = lastTime_.msecsTo(QDateTime::currentDateTime());
...
void VideoThread::checkOnlyAudio() {
    ...
    if (videoCore == VideoCore_Mpv && mediaType == MediaType_FileLocal) {
        if (videoWidth_ == 75 && videoHeight_ == 75) {
            onlyAudio_ = true;
        }
    }

    emit receivePlayStart(timer_.elapsed());
}
...
void VideoThread::snapFinsh() {
    ...
    if (snapName_.endsWith(".png")) {
        QFile(snapName_).remove();
        snapName_ = snapName_.replace(".png", ".jpg");
        //vlc内核需要重新旋转
        if (videoCore == VideoCore_Vlc) {
            VideoHelper::rotateImage(rotate_, image);
        }
        image.save(snapName_, "jpg");
    }
...
void VideoThread::recordStartFinsh() {
    ...
    if (fileName_.startsWith("rtmp://") || fileName_.startsWith("rtsp://")) {
        debug("开始推流", QString("地址: %1").arg(fileName_));
    } else {
        debug("开始录制", QString("文件: %1").arg(fileName_));
    }
}
...
void VideoThread::recordStopFinsh() {
    ...
    emit recorderStateChanged(RecorderState_Stopped, fileName_);
    if (fileName_.startsWith("rtmp://") || fileName_.startsWith("rtsp://")) {
        debug("结束推流", QString("地址: %1").arg(fileName_));
    } else {
        debug("结束录制", QString("文件: %1").arg(fileName_));
    }
}
